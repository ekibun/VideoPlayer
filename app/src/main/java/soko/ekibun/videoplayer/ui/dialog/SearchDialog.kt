...
class SearchDialog<T : ProviderAdapter.ProviderInfo>(val context: ProviderAdapter.LineProviderActivity<T>) :
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        adapter.setOnItemClickListener { _, _, position ->
            ...
            val exist =
                adapter.lines?.providers?.firstOrNull { it.site == item.site && it.id == item.id && it.offset == item.offset } != null
            if (exist) {
                Snackbar.make(view.item_snackbar, "线路已存在，长按编辑此线路", Snackbar.LENGTH_LONG).show()
        ...
        adapter.setOnItemLongClickListener { _, _, position ->
            val item =
                adapter.data[position]//?.let { item -> adapter.lines?.providers?.firstOrNull { it.site == item.site && it.id == item.id }?: item }
            showLineDialog(item)
            dismiss()
            true
        }
        ...
        val searchCall =
            ArrayList<Pair<ProviderAdapter.ProviderInfo, JsEngine.ScriptTask<List<LineInfoModel.LineInfo>>>>()
        ...
        view.item_search.setOnClickListener {
            ...
            searchCall.addAll(if (provider == emptyProvider) {
                ArrayList(context.lineProvider.providerList.values.filter { it.search.isNotEmpty() }).filter { it.search.isNotEmpty() }
                    .map {
            ...
            searchCall.forEach {
                it.second.enqueue({ lines ->
                    ...
                    Snackbar.make(view.item_snackbar, "${it.first.title}: ${e.message}", Snackbar.LENGTH_LONG).show()
                })
            }
        }
    ...
    private fun updateProvider(view: View) {
        ...
        val providers: ArrayList<ProviderAdapter.ProviderInfo> =
            ArrayList(videoProvider.providerList.values.filter { it.search.isNotEmpty() })
